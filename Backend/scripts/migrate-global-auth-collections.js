...
async function mergeByPhone(targetCollection, sourceDoc, existingDoc, stats) {
    ...
    for (const key of ['isVerified', 'isPartner', 'termsAccepted']) {
        if (sourceDoc[key] === true && existingDoc[key] !== true) {
            set[key] = true;
        }
    }

    if (isValuePresent(sourceDoc.partnerApprovalStatus) && !isValuePresent(existingDoc.partnerApprovalStatus)) {
        set.partnerApprovalStatus = sourceDoc.partnerApprovalStatus;
    }

    if (isValuePresent(sourceDoc.partnerSince) && !isValuePresent(existingDoc.partnerSince)) {
        set.partnerSince = sourceDoc.partnerSince;
    }

    if (Array.isArray(sourceDoc.savedHotels) && sourceDoc.savedHotels.length) {
        addToSet.savedHotels = { $each: sourceDoc.savedHotels };
    }
    ...
    if (sourceDoc.hotelFcmTokens?.app && !existingDoc.hotelFcmTokens?.app) {
        set['hotelFcmTokens.app'] = sourceDoc.hotelFcmTokens.app;
    }

    if (sourceDoc.hotelFcmTokens?.web && !existingDoc.hotelFcmTokens?.web) {
        set['hotelFcmTokens.web'] = sourceDoc.hotelFcmTokens.web;
    }
