...
function emitDeliveryDropOtpToUser(order, plainOtp) {
    try {
        ...
        io.to(rooms.user(order.userId)).emit('delivery_drop_otp', {
            ...
            otp: plainOtp,
            message: 'Share this OTP with your delivery partner to hand over the order.'
        });
    } catch (e) {
...
export async function createOrder(userId, dto) {
    ...
    // COD (QR) requires Razorpay to be configured because we auto-verify via Razorpay.
    if (isRazorpayQr && !isRazorpayConfigured()) {
    ...
    const payment = {
        ...
        status: isCash ? 'cod_pending' : isWallet ? 'paid' : isRazorpayQr ? 'pending_qr' : 'created',
    ...
    // For cash + COD-QR we can assign delivery partner before payment is collected/verified.
    if (
        dispatchMode === 'auto' &&
        (order.payment.status === 'paid' ||
            order.payment.status === 'cod_pending' ||
            order.payment.status === 'pending_qr')
    ) {
        try {
...
export async function verifyPayment(userId, dto) {
    ...
    if (order.payment.method !== 'razorpay') {
        throw new ValidationError('Payment method mismatch for this order');
    }
    if (!order.payment.razorpay?.orderId) {
        throw new ValidationError('Missing Razorpay order mapping on order');
    }
    if (String(order.payment.razorpay.orderId) !== String(dto.razorpayOrderId)) {
        throw new ValidationError('Razorpay order id mismatch');
    }

    if (
        order.payment.razorpay?.paymentId &&
        String(order.payment.razorpay.paymentId) !== String(dto.razorpayPaymentId)
    ) {
        throw new ValidationError('Order payment already verified with a different payment id');
    }
    ...
    // Extra server-side verification (amount + payment status) to prevent tampering/replay.
    ...
    try {
        ...
        throw new ValidationError(err?.message || 'Failed to fetch Razorpay payment');
    }

    const rzStatus = String(rzPayment?.status || '').toLowerCase();
    const isSuccessful = rzStatus === 'captured' || rzStatus === 'authorized' || rzStatus === 'paid';
    if (!isSuccessful) throw new ValidationError('Razorpay payment not successful');

    const rzOrderId = String(rzPayment?.order_id || rzPayment?.orderId || '');
    if (rzOrderId && String(dto.razorpayOrderId) !== rzOrderId) {
        throw new ValidationError('Razorpay payment order mismatch');
    }

    const rzAmountPaise = Number(rzPayment?.amount || 0);
    if (expectedAmountPaise > 0 && rzAmountPaise !== expectedAmountPaise) {
        throw new ValidationError('Razorpay amount mismatch');
    }

    // Prevent replay across orders (payment id reused).
    const usedElsewhere = await FoodOrder.exists({
        _id: { $ne: order._id },
        'payment.razorpay.paymentId': String(dto.razorpayPaymentId)
    });
    if (usedElsewhere) throw new ForbiddenError('Payment already used for another order');
    ...
    await appendOrderPaymentLedger(order, 'online_payment_verified', {
        ...
        metadata: {
            ...
            razorpayPaymentStatus: rzStatus,
...
export async function tryAutoAssign(orderId) {
    const order = await FoodOrder.findById(orderId).select('restaurantId dispatch payment').lean();
    if (!order || order.dispatch?.status !== 'unassigned') return null;

    // Security gate: for online Razorpay, do not assign delivery until payment is verified.
    if (order.payment?.method === 'razorpay' && order.payment?.status !== 'paid') return null;
    ...
    const filter = {
        $or: [
            // "Marketplace" pool – only show orders once restaurant accepted.
            {
                ...
                // Security: never show unpaid Razorpay orders to delivery partners.
                $or: [{ 'payment.method': { $ne: 'razorpay' } }, { 'payment.status': 'paid' }]
            },
...
export async function acceptOrderDelivery(orderId, deliveryPartnerId) {
    ...
    // Security gate: only allow delivery claiming once online Razorpay payment is verified.
    if (order.payment?.method === 'razorpay' && order.payment?.status !== 'paid') {
        throw new ValidationError('Online payment not verified yet');
    }
...
export async function confirmReachedDropDelivery(orderId, deliveryPartnerId) {
    ...
    const fromPhase =
        order.deliveryState?.status || order.deliveryState?.currentPhase || order.orderStatus || '';
...
export async function verifyDropOtpDelivery(orderId, deliveryPartnerId, otp) {
    ...
    if (!order.deliveryVerification?.dropOtp?.required) {
        throw new ValidationError('OTP verification is not active for this order. Confirm reached drop first.');
    }
    if (order.deliveryVerification?.dropOtp?.verified) {
...
export async function completeDelivery(orderId, deliveryPartnerId) {
    ...
    // - `razorpay`: must already be verified/paid before completing delivery
    // - `razorpay_qr`: we auto-verify Razorpay QR link status before completing delivery
    // - `cash`: rider marks as paid on successful delivery
    if (payMethod === 'razorpay') {
        if (order.payment.status !== 'paid') throw new ValidationError('Online payment not verified');
    } else if (payMethod === 'razorpay_qr') {
        await syncRazorpayQrPayment(order);
        if (order.payment.status !== 'paid') throw new ValidationError('QR payment not verified');
    } else if (payMethod === 'wallet') {
        if (order.payment.status !== 'paid') throw new ValidationError('Wallet payment not verified');
    }
    ...
    if (!shouldSkipQrLedger) {
        ...
        await appendOrderPaymentLedger(order, ledgerKind, {
            ...
            metadata: { previousPaymentStatus: prevPayStatus, orderStatus: 'delivered' }
        });
    }
...
function emitOrderUpdate(order, deliveryPartnerId) {
    try {
        const io = getIO();
        if (io) {
            const dv = order.deliveryVerification?.toObject?.() || order.deliveryVerification;
            const payload = {
                orderMongoId: order._id?.toString?.(),
                orderId: order.orderId,
                orderStatus: order.orderStatus,
...
export async function createCollectQr(orderId, deliveryPartnerId, customerInfo = {}) {
    ...
    if (!order) throw new ValidationError('Order not found');
    if (order.dispatch.deliveryPartnerId?.toString() !== deliveryPartnerId.toString()) throw new ForbiddenError('Not your order');
    if (order.payment.status === 'paid') throw new ValidationError('Order already paid');
    const payMethod = order.payment?.method;
    // QR collection is only allowed for COD orders (or if QR was already initiated).
    if (payMethod !== 'cash' && payMethod !== 'razorpay_qr') {
        throw new ValidationError('QR collection is not allowed for this payment method');
    }

    // Idempotency: if we already generated a QR for this order and it is still pending, reuse it.
    if (
        ...
        return {
            shortUrl: order.payment.qr.shortUrl || null,
            imageUrl: order.payment.qr.imageUrl || order.payment.qr.shortUrl || null,
            amount: order.payment.amountDue ?? order.pricing?.total ?? 0,
...
/**
 * Auto-verify Razorpay QR (payment-link status) and sync order.payment.status.
 * This is the server-side "auto verify" mechanism (no client trust).
 */
async function syncRazorpayQrPayment(orderDoc) {
    if (!orderDoc?.payment || orderDoc.payment.method !== 'razorpay_qr') return orderDoc?.payment;
    ...
    try {
        ...
        // If Razorpay fetch fails, keep payment as-is (caller may retry).
        logger.warn(`Razorpay paymentLink fetch failed: ${err?.message || err}`);
    ...
    // Update QR snapshot status even if not paid (helps UI / debugging).
    orderDoc.payment.qr = {
        ...(orderDoc.payment.qr?.toObject?.() || orderDoc.payment.qr || {}),
        status: linkStatus
    };

    const expectedAmountPaise = Math.round(Number(orderDoc.payment.amountDue || 0) * 100);
    const linkAmountPaise = link?.amount != null ? Number(link.amount) : null;
    const amountMatches = expectedAmountPaise > 0 && linkAmountPaise != null ? linkAmountPaise === expectedAmountPaise : true;

    if (linkStatus === 'paid' && amountMatches) {
        orderDoc.payment.status = 'paid';
        await orderDoc.save();

        await appendOrderPaymentLedger(orderDoc, 'payment_snapshot_sync', {
            recordedByRole: 'SYSTEM',
            metadata: {
                paymentLinkId,
        ...
        // Do not auto-complete delivery if QR payment never completed.
...
export async function getPaymentStatus(orderId, deliveryPartnerId) {
    const order = await FoodOrder.findById(orderId).select('payment dispatch').exec();
    if (!order) throw new ValidationError('Order not found');
    if (order.dispatch?.deliveryPartnerId?.toString() !== deliveryPartnerId.toString()) throw new ForbiddenError('Not your order');

    // Server-side QR auto-verify.
