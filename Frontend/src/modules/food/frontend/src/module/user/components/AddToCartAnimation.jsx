...
export default function AddToCartAnimation({
  ...
  const iscartPage = location.pathname === '/food/cart' || location.pathname.startsWith('/food/cart/');
  const isOrderPage = location.pathname.startsWith('/food/orders/');
      ...
      <AnimatePresence>
        {itemCount > 0 && !shouldHidePill && (
          <motion.div
            ...
            className={`${dynamicBottom ? `fixed ${dynamicBottom}` : 'sticky bottom-0'} left-0 right-0 z-40 flex justify-center px-4 pb-4 md:pb-6 transition-all duration-300 ease-in-out`}
          >
            <Link
              ref={linkRef}
              to={linkTo}
